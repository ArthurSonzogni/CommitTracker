...
export default {
  mounted: async function() {
    ...
    const contributors_per_year = {};
    for(const user of Object.values(data)) {
      for(const year in user.author.by_date) {
        contributions_per_year[year] ||= 0;
        contributions_per_year[year] += user.author.by_date[year];
        contributors_per_year[year] ||= 0;
        contributors_per_year[year] += 1
      }
    }
    for(const year in contributions_per_year) {
      per_year[year] =
        contributions_per_year[year] /
        contributors_per_year[year];
    }
