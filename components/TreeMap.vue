...
const renderRect = function(rect, x, y, hover) {
  return rect
    ...
    .style("fill", d => {
      ...
      const c1 = "white";
      const c2 = props.colormapFunc(colorScale(color / size));
...
const render = function(group, data, x, y) {
  ...
  child
    .on("mousemove", (event, d) => {
      ...
      select(componentName.value.$el).text(d.data.name);
...
const resize = function() {
  try {
    ...
    svgHeight.value = svgWidth.value * aspect_ratio - 200;
  } catch (e) {
    console.log(e);
  }
  this.paramsChanged()
...
const path_wrapped = computed(() => {
  return [...this.path];
});

watch(repositories, paramsChanged);
watch(field_size, paramsChanged);
watch(field_color, paramsChanged);
watch(colormap, refresh);
watch(colormapMin, refresh);
watch(colormapMax, refresh);
